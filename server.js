...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.comment && (requestComment.comment.body) && database.articles[requestComment.comment.articleId] && database.users[requestComment.comment.username]) {
    const comment = {
...
function updateComment(url, request) {
  ...
  if (!savedComment || !requestComment) {
    response.status = 400;
...
function loadDatabase() {
  try {
    const file_yaml = fs.readFileSync('database.yml', 'utf8');
    const db_json = js_yaml.safeLoad(file_yaml);
    console.log(db_json);
    return db_json;
  } catch (error) {
    console.log(error)
...
function saveDatabase() {
  try {
    const db_yaml = js_yaml.safeDump(database);
    fs.writeFile('database.yml', db_yaml, (error) => {
